...
char *Stdin_Buffer_Contents(size_t *return_len) {
  char *ret;
  long len, outcome;
  fseek(stdin, 0, SEEK_END);  // Move cursor to end of stdin contents.
  len = ftell(stdin);
  ...
  fseek(stdin, 0, SEEK_SET);  // Set cursor back to beginning.
  ...
  if (len != (outcome = fread(ret, sizeof(char), len, stdin))) {
    perrf("Failed to read full stdin contents. Got %lu chars out of %ld\n"
  ...
  // Even though we calloc'd, just null terminate it manually, just in case.
  ret[(*return_len = len)] = '\0';
...
void BST_Advance_Curr_State_Set_And_Dealloc_Prev(BST_t *curr, BST_t *prev, BST_t **delta, char alpha_sym) {
  BST_Node_t *stack[curr->size], *popped, *tmp;
  int alpha_idx = alpha_sym - 'a' + 1;
  int size = curr->size;
  int top = -1;
  if (NULL == (tmp = curr->root)) {
    assert(0 == (curr->size));
    return;
  ...
  while (top > -1) {
    popped = stack[top--];
    if (NULL != (tmp = (popped->l))) {
      stack[++top] = tmp;
    }
    
    if (NULL != (tmp = (popped->r))) {
      stack[++top] = tmp;
    }

    BST_Combine(prev, &delta[popped->state][alpha_idx]);
    free(popped);
    --size;
  }
...
_Bool NFA_Simulate(ENFA_t *nfa, const char *input) {
  BST_t bufa = {0}, bufb = {0}, **delta = nfa->delta, *finals = &(nfa->final_states), *curr, *prev, *swapper = NULL;
  int alpha_len = nfa->alpha_len;
  char c, tmp;
  _Bool ret = 0;
  prev = &bufb;
  BST_Add((curr = &bufa), 0);
  while ((c = *input++)) {
    if (curr->size == 0)
      break;
    if ((tmp = c-'a') < 0 || tmp > alpha_len) {
      BST_Close(curr);
      return 0;
    }
    BST_Advance_Curr_State_Set_And_Dealloc_Prev(curr, prev, delta, c);
    swapper = curr;
    curr = prev;
    prev = swapper;
  }
  ret = BST_CheckOverlap(finals, curr);
  BST_Close(curr);
  return ret;
...
int main(int argc, char *argv[]) {
  ...
  enfa = Parse_NFA_File(fp);

  fclose(fp);
  
  if (NULL == enfa)
    return -1;
  if (NULL == (stdin_buffer = Stdin_Buffer_Contents(&stdin_buf_len))) {
    Dealloc_ENFA(enfa);
    return -1;
  }

  char *cursor = stdin_buffer, *cursor_savept, tmp;
  _Bool hasmore;
  for (;;) {
    cursor_savept = cursor;
    do {
      tmp = *(cursor++);
    } while ((hasmore = tmp) && tmp != '\n');
    if (!hasmore)
      break;
    cursor[-1] = '\0';
    if (NFA_Simulate(enfa, cursor_savept)) {
      puts("accept");
    } else {
      puts("reject");
    }
